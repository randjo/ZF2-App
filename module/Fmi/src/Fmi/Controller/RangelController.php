...
use Fmi\Entity\User1;
/**
 * FMI controller
 *
 * This controller has been build with <b>educational</b> purposes
 */
class RangelController extends AbstractActionController
{
    /**
    ...
    public function indexAction()
    {
		...
		$dql = "SELECT u FROM Fmi\Entity\User1 u";
		
		$query = $entityManager->createQuery($dql);
		$query->setMaxResults(30);
		$users = $query->getResult();
		//$this->layout('layout/rangel');
                //$this->layout('layout/NikolaVasilev');
		return new ViewModel(array('users1' => $users));
    }
    public function addAction()
    {
		...
		$form = $builder->createForm( $user1 );
		$form->setHydrator(new DoctrineHydrator($entityManager,'Fmi\Entity\User1'));
        ...
        if ($request->isPost()) {
            ...
            if ($form->isValid()) { // if it is valid hets populated
                ...
                return $this->redirect()->toRoute('fmi/default', array('controller' => 'Rangel', 'action' => 'index'));
            }
        }
        //$this->layout('layout/NikolaVasilev');
        //$this->layout('layout/rangel');
    ...
    public function editAction()
    {
        ...
        if (!$id) {
            return $this->redirect()->toRoute('fmi/default', array('controller' => 'Rangel', 'action' => 'add'));
        }
        ...
        try {
			...
			$repository = $entityManager->getRepository('Fmi\Entity\User1');
			// $id = (int)$this->params()->fromQuery('id', 1);
			$user1 = $repository->find($id);
        }
        catch (\Exception $ex) {
            return $this->redirect()->toRoute('fmi/default', array('controller' => 'Rangel',
                'action' => 'index'
            ));
        }
		// Create the form
    ...
    public function deleteAction()
    {
        ...
        return array(
            'id'    => $id,
            'user1' => $user1, // $this->getAlbumTable()->getAlbum($id)
        );	
	}
